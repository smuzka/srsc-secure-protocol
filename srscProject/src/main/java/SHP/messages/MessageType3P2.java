...
public class MessageType3P2 extends Message {
    ...
    private byte[] nonce4;
    private int udpPort;
    private byte[] ydhClient;

    public MessageType3P2(byte[] salt, byte[] counter, byte[] nonce3_previous, String userId) {
        this.salt = salt;
    ...
    public MessageType3P2(String password, byte[] salt, byte[] counter, String request, String userId, byte[] nonce3,
            byte[] nonce4, int udpPort, byte[] ydhClient) {
    ...
    private byte[] createSerializedPayload() {
        ...
        byte[] ydhClientSerialized = Serializer.serializeBytes(ydhClient);

        return Util.mergeArrays(requestSerialized, userIdSerialized, nonce3Serialized, nonce4Serialized,
                udpPortSerialized, ydhClientSerialized);
    }
    ...
    private void decryptAndDeserializeData(byte[] data) {
        ...
        Serializer<byte[]> ydhClientDeserialized = Serializer
                .deserializeFirstBytesInArray(udpPortDeserialized.getRemainingBytes());
        this.ydhClient = ydhClientDeserialized.getExtractedBytes();
    }
    ...
    public String toString() {
        return "MessageType3: " + "request=" + request + ", userId=" + userId + ", nonce3=" + Arrays.toString(nonce3)
                + ", nonce4=" + Arrays.toString(nonce4) + ", udpPort=" + udpPort + ", ydhClient="
                + Arrays.toString(ydhClient);
    }
}
